        ...
        onAuthStateChanged(auth, (user) => {
            if (user && user.email === "ricute069@gmail.com") document.body.classList.add('is-admin');
        ...
        if (readingMode === 'focus') {
            ...
            if (miniTimerBtn) {
                miniTimerBtn.onclick = async () => {
                    const isConfirm = await window.showConfirm("Bạn có muốn tạm dừng và quay lại bảng điều khiển Focus?");
                    if (isConfirm) {
                        clearInterval(focusTimerInterval);
                        isTimerRunning = false;
                        focusState.isRunning = false;
                        ...
                        if (startBtn && !focusState.needsBreak) startBtn.textContent = "Đọc tiếp";
                    }
                };
            }
